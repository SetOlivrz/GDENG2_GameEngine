...
void InspectorScreen::updateTransform()
{
    ...
    vector<AComponent*>physicsComp = obj->getComponentsOfType(AComponent::ComponentType::Physics);
    if (physicsComp.size()>0)
    {
        for (int i = 0; i < physicsComp.size(); i++)
        {
            ((PhysicsComponent*)physicsComp.at(i))->updateRigidBody();
       }
    }
